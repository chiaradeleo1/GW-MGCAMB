    ...
    real(dl) gwlens_volume_source, gwlens_sw_source !CDL
        ...
        associate (W => State%Redshift_W(w_ix))
            ...
            elseif (W%kind == window_gwlens) then !CDL
                ...
                if (CP%SourceTerms%gwlens_volume) then
                    ...
                    !Sachs-Wolfe source 
                if (CP%SourceTerms%gwlens_sw) then
                        
                    if (MG_flag==0) then
                            
                        gwlens_sw_source = W%wing(j)/adotoa/(State%tau0 - tau)*phi
                        
                    else
                        gwlens_sw_source = W%wing(j)/adotoa/(State%tau0 - tau)*mg_psi !CDL MG
                    end if
                else
                    gwlens_sw_source = 0._dl
                    
                end if

                sources(3+w_ix)= gwlens_volume_source + gwlens_sw_source
                sources(3+w_ix)=sources(3+w_ix)/W%Fq
